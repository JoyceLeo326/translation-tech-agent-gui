...
def replace_in_xml_paragraphs(
    ...
    for p_elem in xml_root.iter(p_tag):
        ...
        if best_match:
            ...
            if total_chars > 0 and len(t_elems) > 1:
                # 按比例分配英文到各个run
                # 先计算每个run的起始位置比例
                run_ratios = []
                current_pos = 0
...
def process_xml_file(zin: zipfile.ZipFile, xml_path: str, sorted_keys: list, cn_to_en: dict, matched_keys: Set[str]):
    """处理单个XML文件，返回修改后的XML bytes、替换段落数、中文残留片段。"""
    raw_xml = zin.read(xml_path)
    root = ET.fromstring(raw_xml)
    replaced_count = replace_in_xml_paragraphs(root, sorted_keys, cn_to_en, matched_keys)
                ...
                if should_process and fname in all_files:
                    ...
                    try:
                        modified_xml, replaced_count, residue = process_xml_file(
                            zin, fname, sorted_keys, cn_to_en, matched_keys
                        )
