...
def generate_excel_node(state: GenerateExcelInput, config: RunnableConfig, runtime: Runtime[Context]) -> GenerateExcelOutput:
    ...
    # 去除[TITLE]标记（标题标记，仅用于内部处理）
    def remove_title_marker(text):
        """去除[TITLE]标记"""
        if text.startswith('[TITLE]'):
            return text[7:]  # 去除[TITLE]（7个字符）
        return text

    cn_items = [(num, remove_title_marker(content)) for num, content in cn_items]
    en_items = [(num, remove_title_marker(content)) for num, content in en_items]
