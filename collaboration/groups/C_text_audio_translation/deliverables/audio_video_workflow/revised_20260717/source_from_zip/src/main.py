...
async def _global_exception_middleware(request: Request, call_next):
    """
    全局异常兜底中间件 - 防止"Pending response rejected"等 ASGI/HTTP 协议层错误
    影响用户的工作流结果。

    覆盖场景:
      1) StarletteClientDisconnect: 客户端主动断开(扣子前端/AI 客户端)
      2) RuntimeError: 含"Pending response rejected"、"No response returned"等
      3) OSError: 连接重置/管道破裂
      4) asyncio.CancelledError: 任务被取消
      5) BaseException: 兜底(防止任何未处理异常导致服务挂掉)
    """
    try:
        return await call_next(request)
    except StarletteClientDisconnect:
        # 客户端主动断开(扣子前端/AI 客户端关闭页面) - 静默结束
        run_id_hint = "?"
        try:
            from coze_coding_utils.runtime_ctx.context import request_context as _rc
            if _rc.get():
                run_id_hint = _rc.get().run_id
        except Exception:
            pass
        logger.info(f"客户端在请求过程中断开连接 (path={request.url.path}, run_id={run_id_hint})")
        # 客户端已经断开,返回什么都不会被看到,只兜底防止 500
        return Response(status_code=499, content=b"client closed")
    except RuntimeError as e:
        # 捕获 "Pending response rejected since connection got disposed"
        # "No response returned" 等 ASGI 内部错误
        err_str = str(e)
        if any(s in err_str for s in (
            "Pending response",
            "No response returned",
            "connection got disposed",
            "response got disposed",
        )):
            logger.warning(
                f"ASGI 协议层连接错误(已吞掉,不影响工作流结果): {err_str[:200]} "
                f"[path={request.url.path}]"
            )
            return Response(status_code=499, content=b"asgi connection closed")
        # 其他 RuntimeError 仍然往上抛,让上层 except 处理
        raise
    except (ConnectionError, OSError, BrokenPipeError) as e:
        # TCP 连接断开
        logger.warning(
            f"TCP 连接错误(已吞掉): {type(e).__name__}: {str(e)[:200]} "
            f"[path={request.url.path}]"
        )
        return Response(status_code=499, content=b"connection closed")
    except asyncio.CancelledError:
        # 任务取消 - 正常
        logger.info(f"请求被取消 [path={request.url.path}]")
        raise
    except Exception as e:
        # 兜底:任何未处理异常都不让服务挂掉
        logger.error(
            f"全局未处理异常(已兜底,500 返回): {type(e).__name__}: {str(e)[:200]} "
            f"[path={request.url.path}]",
            exc_info=True
        )
        try:
...
async def _safe_stream_wrapper(
    ...
    当客户端断开连接时,SSE 流会被框架异步终止,生成器可能在 yield 时
    收到上述异常。这些都是客户端正常断开信号,不应该作为错误抛出。
    """
    try:
        async for chunk in stream_generator:
            yield chunk
    except (StarletteClientDisconnect, asyncio.CancelledError):
        # 客户端主动断开连接 - 正常结束
        logger.info(f"SSE stream client disconnected for run_id: {run_id}, ending gracefully")
        return
    except (RuntimeError, OSError, ConnectionError) as e:
        # 关键修复: 捕获 ASGI/Pending response rejected 等连接层异常
        # 这些异常表示客户端连接已断,工作流结果无法推送,但服务端无需报错
        error_msg = str(e)[:200]
        if (
            "disposed" in error_msg.lower()
            or "rejected" in error_msg.lower()
            or "disconnect" in error_msg.lower()
            or "no response" in error_msg.lower()
        ):
            logger.info(
                f"SSE stream connection disposed for run_id: {run_id}, "
                f"error (suppressed): {error_msg}"
            )
            return
        # 真正的 RuntimeError - 重新抛出
        logger.error(f"SSE stream RuntimeError for run_id: {run_id}, error: {error_msg}")
        raise
    except GeneratorExit:
        # 生成器被关闭 - 客户端断开
        logger.info(f"SSE stream GeneratorExit for run_id: {run_id}, ending gracefully")
        return
    except Exception as e:
        # 其他异常 - 记录但尝试优雅退出
        error_msg = str(e)[:200]
        logger.warning(
            f"SSE stream unexpected exception for run_id: {run_id}, "
            f"error: {error_msg}, traceback: {traceback.format_exc()[:500]}"
        )
        # 不重新抛出,让工作流在服务端正常完成
        return
    finally:
        # 清理 task 注册
        if run_id in service.running_tasks:
            del service.running_tasks[run_id]
...
async def http_stream_run(request: Request):
    ...
    if is_agent:
        raw_generator = agent_stream_handler(
            payload=payload,
            ctx=ctx,
            run_id=run_id,
        ...
        raw_generator = workflow_stream_handler(
            payload=payload,
            ctx=ctx,
            run_id=run_id,
            stream_sse_func=service.stream_sse,
    ...
    # ⭐ 修复 "Pending response rejected since connection got disposed":
    # 包一层 safe_stream_wrapper,客户端断开时优雅退出
    stream_generator = _safe_stream_wrapper(raw_generator, run_id=run_id)
...
async def openai_chat_completions(request: Request):
    ...
    try:
        ...
        # 客户端断开/任务取消 - 正常信号,不抛 500
        logger.info(f"OpenAI 客户端断开或取消 [run_id={ctx.run_id}]")
        return Response(status_code=499, content=b"client closed")
    except RuntimeError as e:
        # "Pending response rejected since connection got disposed"
        # 客户端已经断开连接,吞掉这个错
        err_str = str(e)
        if any(s in err_str for s in (
            "Pending response", "No response returned",
            "connection got disposed", "response got disposed",
        )):
            logger.warning(
                f"OpenAI SSE 协议层连接错误(已吞掉): {err_str[:200]} [run_id={ctx.run_id}]"
            )
            return Response(status_code=499, content=b"asgi closed")
        raise
    finally:
        cozeloop.flush()
