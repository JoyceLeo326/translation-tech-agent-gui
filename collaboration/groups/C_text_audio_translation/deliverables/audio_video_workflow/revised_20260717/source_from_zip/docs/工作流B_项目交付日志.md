...
1. **二维码图片下载链接**：有效期 24 小时（`expire_time=86400`），过期后无法下载 PNG
2. **二维码内嵌音频播放链接**：有效期 24 小时，过期后扫码无法播放
3. **分段音频 URL**：有效期 24 小时，过期后无法下载
4. **重新运行工作流**：每次运行生成全新 URL，旧 URL 立即失效（新旧 URL 互不影响，各自独立过期）
5. **归档建议**：如需长期保存音频，应下载 MP3 文件到本地或自有存储，不可仅依赖预签名 URL
