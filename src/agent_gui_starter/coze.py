...
class CozeWorkflowClient:
    ...
    def run(self, input_text: str, input_title: str = "") -> CozeWorkflowResponse:
        ...
        if not text:
            raise CozeWorkflowError("Coze 工作流需要输入待翻译正文。")

        if not self._config.has_coze_workflow:
            return CozeWorkflowResponse(
                text=(
                    "当前未配置 Coze 访问令牌，因此没有向平台发送网络请求。\n\n"
        ...
        try:
            ...
            raise CozeWorkflowError(
                f"Coze API 请求失败（HTTP {exc.code}）：{_shorten(detail, 400)}"
            ) from exc
        except URLError as exc:
            raise CozeWorkflowError(f"无法连接 Coze API：{exc.reason}") from exc
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise CozeWorkflowError("Coze API 返回了无法解析的响应。") from exc

        code = payload.get("code", 0)
        if code not in (0, "0", None):
            message = str(payload.get("msg") or payload.get("message") or "未知错误")
            raise CozeWorkflowError(f"Coze 工作流执行失败（{code}）：{message}")

        output = _extract_workflow_text(payload.get("data"))
        if not output:
            raise CozeWorkflowError("Coze 工作流执行成功，但没有返回可显示的最终文本。")
